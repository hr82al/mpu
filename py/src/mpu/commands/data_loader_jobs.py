"""`mpu data-loader-jobs <method>` — обёртки node cli
service:dataLoaderJobs (exec через Portainer; `--print` — печать)."""
...
COMMAND_SUMMARY = "Обёртки service:dataLoaderJobs (exec через Portainer)"

app = typer.Typer(
    no_args_is_help=True,
    context_settings={"help_option_names": ["-h", "--help"]},
    help=(
        "Обёртки над `node cli service:dataLoaderJobs` (sl-back). Дефолт — немедленное "
