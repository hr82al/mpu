...
Корневой CLAUDE.md монорепо (`../../CLAUDE.md`) применяется как есть: token economy для CLAUDE.md, sandbox-правила, семь принципов (думать до кода / минимум кода / хирургические правки / цели вместо процесса / экономия токенов / читаемость / финальный проход по готовому коду), language policy, MR-workflow. `../CLAUDE.md` (`mpu/CLAUDE.md`) — overview дерева `mpu` целиком (`py/` + `ts/`) и правило коммита прямо в main.
...
Поверх корневых принципов монорепо.
...
8. **Читаемость и качество — приоритет над краткостью и скоростью.** Ясность и сопровождаемость важнее «покороче» / «поскорее». Новый код пишется как `commands/kiten/timelog.py` — образец стиля дерева (чистые хелперы отдельно от эффектов, типы на границах, ранние выходы); образец устарел — заменить ссылку, а не оставлять её. Дополняет корневой принцип «минимум кода» (тот — против спекулятивных фич; этот — против жертвы ясностью ради краткости), не противоречит ему.
   ...
   - **8.1 Бюджеты.** Тело команды ≤ 40 строк, функция ≤ 60, модуль ≤ 400, вложенность ≤ 4. Превысил — расслоить. `# noqa` на `C901` / `PLR0912` / `PLR0915` в новом коде не ставить: это сигнал декомпозиции, а не шум линтера. Число параметров не лимитируется (`PLR0913` глушить можно): лимит аргументов гонит к объекту-мешку состояния, который читается хуже длинной сигнатуры. Аннотации опций в бюджет тела не входят, но см. 8.2.
   ...
   - **8.3 Второй копии не бывает.** Понадобилось скопировать функцию или блок в соседний модуль — вместо копии общая точка (`commands/_<name>.py` / `lib/<name>.py`). Копии расходятся молча: в репо есть пара команд, где одинаковый резолв номера сервера различается граничным условием, и несколько копий разбора A1-кавычек, не экранирующих апостроф, — хотя корректный вариант лежит в `lib/sheet_batch.py` (снятие кавычек через `.replace("''", "'")`). Адреса не фиксировать номерами строк — они устаревают; искать по символу.
   - **8.3.1 Данные, а не источник.** Функция принимает то, что ей нужно (строки, значения, готовые записи), а не объект-источник (`Connection`, `Store`, `httpx.Client`, `EnvFile`), из которого достаёт нужное сама. Понадобился мок — это признак лишней зависимости в коде, а не недостающей фикстуры в тесте. Публичный API при этом не расширяется ради тестов: экспортировать внутренний хелпер, чтобы вызвать его напрямую, — не решение.
...
5. Каждое имя: что оно обещает и что код делает на самом деле?
6. Каждая граница диапазона и каждое сравнение: включительно или нет — и что требует контракт (докстрока, `--help`)?

Если на любой вопрос ответ «не знаю» — добавить тест или починить.

## Разбор итогового диффа

Порядок: **гейты зелёные → разбор диффа → правки → гейты повторно → коммит.** Разбор после коммита не считается.

Разбор нетривиального диффа делает **свежий контекст** — субагент, которому дают только `git diff` и текст задачи, без истории написания. Замер: разбор своего кода дал четыре находки, реальных из них ноль (все описывали дефекты, уже исправленные по дороге), свежий контекст — одну находку, и она была реальной.

В разбор идёт **только то, что есть в итоговом диффе**: дефект, найденный и исправленный по ходу написания, находкой не считается — иначе отчёт выглядит проверенным, не будучи им.

Глубина по риску: дифф меняет поведение, контракт команды или обработку данных — полный разбор по вопросам «Самопроверки» выше. Переименование, конфиг, правка без изменения логики — строка «тривиальный дифф (N строк, логика не менялась), гейты зелёные».

Форма: `файл:строка — в чём нарушение — как переписать`. «Нарушений нет» — только вместе с перечислением проверенного. Замечания к соседнему коду — отдельным списком, без правок; проблемы ради находок не искать.
