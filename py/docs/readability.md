...
**✅ Сведено (шаг 12, с решением владельца «слить дежурные, оставить контекстные»).**
`--json` — 11 формулировок → 3: 24 места через общий `cli_opts.JsonOpt` («JSON-вывод вместо
таблицы»), контекстные `Сырой JSON (card + comments)` и `список FileDiff` оставлены. `--dry-run`
13 → 10: семь дежурных сведены, но «не писать в SQLite» / «не call Miro API» / «не отправлять
requests[]» / «ничего не выполнять» оставлены — они описывают, ЧТО именно команда не сделает.
Даты: дежурные (`iu_wb`, `ozon_fix_fo_tax`) приведены к стандарту, а `kiten ls`
(`updated_after`-фильтр) и `time ls` (граница даты записи) оставлены — это разный смысл.
`process --date-to` оставлен без «по умолчанию — сегодня», потому что он туда не дефолтит.
Справка изменилась только в этих местах (проверено diff'ом `--help` всех 164 команд).
...
- `cli.py init_cmd` — пять шагов первичной настройки (схема, Portainer, Loki, Kaiten, Telegram)
  шли одним телом на 105 строк; каждый вынесен в свою функцию, команда — 33 строки, где шаги
  видны списком вызовов;
- `timelog.py time_stop` — вынесены резолв роли (`_stop_role_id`) и печать итога
  (`_render_stop_result`); сценарий остановки таймера остался цельным, дробить его внутри
  `try`/`except KaitenAPIError` смысла нет.

Осталось: `sheet.py set_` / `batch_*` — их разумно делать вместе с §10 под diff-оснасткой,
поскольку они и есть точки входа компилятора мини-языка. Объём: M на команду · выигрыш: high.
...
| ✅ 9 | Свести пролог резолва в `pick_client_id`/`pick_spreadsheet_id` — сделано (17 блоков в 17 файлах). `make_app` НЕ расширяли: инвентарь показал, что 14 обёрток уникальны по форме флагов, третья ветка фабрики дала бы копию ради 15 строк | L | high |
| ✅ 10 | `sheet_batch.py`: `_skip_quoted` (3 автомата → 1) + `compile_read` разбит на `_parse_get`/`_parse_read`; `d2_parser.py`: `parse_d2_source` → список `_try_*`-хелперов — сделано под diff-оснасткой (обе: 0 расхождений на 20k входов), сняты 4 `noqa: C901/PLR0912` | L | medium |
| ✅ 11 | Ленивое монтирование команд — сделано (старт 0,87 → 0,30 с) | S | (perf) |
| ✅ 12 | Свести дежурные формулировки — сделано (`--json` 11→3 через общий `JsonOpt`, `--dry-run` 13→10, даты нормализованы). Контекстные (kiten-фильтры updated_after, «не выполнять/писать», «не отправлять requests[]») оставлены осознанно; справка изменилась только в размеченных местах | M | medium |
...
Аналогичная оснастка для парсера d2 — `scripts/difftest_d2_parser.py` (`parse_d2_source` пишет
в Miro): генерирует d2 по грамматике плюс подмешивает корпус из тестов, проверена мутацией
(перестановка src/dst у `Edge` → 2009 расхождений из 5000).
