...
Источники stdin (по аналогии с `mpu-sql` / `mpu-run-js`):
  ...
  3. pipe: `cat x | mpup-ssh ...` (stdin не TTY);
...
Примеры:
  mpup-ssh sl-1 --no-stdin -- ls -la /app
  mpup-ssh 12345 --no-stdin -- ls /app           # client_id → server через mpu-search
  mpup-ssh "Тортуга" --no-stdin -- ls /app       # title → server через mpu-search
  cat script.mjs | mpup-ssh sl-11 -- node --input-type=module -
  mpup-ssh sl-11 --stdin-text 'console.log(1)' -- node --input-type=module -
  mpup-ssh sl-1 --via portainer --no-stdin -- ls /app
  mpup-ssh sl-11 -- cat   # TTY: введите данные, завершите Ctrl+D
"""
