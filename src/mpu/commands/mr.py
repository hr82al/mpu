"""`mpu mr <comment|note|comments|show|reply|edit|delete|resolve|unresolve>` — ревью GitLab MR.

Инлайн-комментарии под строками диффа merge-request'а и работа с тредами ревью.
Зеркальные сценарии:

- ревьюер: `comment FILE:LINE` — комментарий под строкой кода; `note` — общий
  комментарий MR; `edit`/`delete` — правка своих нот; `reply`/`resolve`;
...
COMMAND_NAME = "mpu mr"
COMMAND_SUMMARY = (
    "GitLab MR code-review: `comment FILE:LINE` — инлайн-комментарий под строкой диффа; "
    "`comments`/`show` — треды ревью; `reply`/`edit`/`delete`/`resolve`"
)
