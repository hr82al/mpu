"""`mpu mr <view|files|diff|comment|note|comments|show|reply|edit|delete|resolve|unresolve>`.

Чтение и ревью GitLab merge-request'а. Read-команды (`view`/`files`/`diff`) дают
контекст для ревью без `glab`/curl; пишущие — инлайн-комментарии и треды.
Зеркальные сценарии:
...
COMMAND_NAME = "mpu mr"
COMMAND_SUMMARY = (
    "GitLab MR code-review: `view`/`files`/`diff` — чтение MR; `comment FILE:LINE` — "
    "инлайн-комментарий под строкой диффа; `comments`/`show` — треды ревью; "
