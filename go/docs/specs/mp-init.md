...
Снять при переводе в «к реализации»: `mpu mp-init --help`;
`mpu mp-init --dry-run` при поднятом окружении (полный список команд);
`mpu mp-init` без каталога mp-config-local (`MPU_MP_CONFIG_LOCAL` на
несуществующий путь, exit 2).
