...
Статус: черновик (сторона — брокер: транспорт исполняется с его
сервера, пути env-файла и ssh-ключей — окружение учётной записи
брокера; формулировки приводятся при переводе в «к реализации»)
...
Env-файл (контракт — `platform/config.md`): `PORTAINER_API_KEY`,
`PORTAINER_VERIFY_TLS`, `sl_<N>`, `PG_MY_USER_NAME`,
`sl_<N>_portainer`, `DEV_NODE_HOST`, `DEV_NODE_USER`. ssh-ключ —
`~/.ssh/id_rsa`, без настройки. Кэш-БД — `platform/config.md`.
