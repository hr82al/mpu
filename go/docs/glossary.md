# Глоссарий — доменный термин → каноничное имя в Go-коде

Изолированные сессии не видят кода друг друга: имена доменных понятий
берутся отсюда, иначе дерево разъедется. Новый термин в спеке — новая
строка здесь (вносит сессия-спецификатор; Go-сессия, вынужденно
выбравшая имя сама, помечает его в отчёте).

| Термин | Go-имя | Значение |
| --- | --- | --- |
...
| журнал вызовов | `InvokeLog`, `RunID` | одна запись на процесс `mpu`; `run_id` = время+pid |
| legacy-делегация | `LegacyDelegation` | операция брокера: исполнение не переехавшей команды Python-mpu |
| операция | `Operation` | именованный JSON-RPC-метод брокера (`<домен>.<действие>`) |
| реестр команд | `Registry` | единый список команд для справки и маршрутизации |
