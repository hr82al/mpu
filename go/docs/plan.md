...
CLI — Deno/TS. Детали и открытые вопросы — `mpu/docs/
broker-architecture.md` (вне `go/`, для сессии-спецификатора). Спеки
...
Порядок «сначала обвязка, затем xlsx» сохраняется по смыслу, но состав
обвязки делится между брокером и CLI — пересмотр впереди.
...
- **dispatcher** — таблица «команда → Go | Python», вызов Python-ветки.
  Спека: черновик (пересмотр под брокер-архитектуру, см. шапку; golden
  сняты 2026-07-29 и остаются эталоном).
