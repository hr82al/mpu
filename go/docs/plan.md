...
протокол (`specs/protocol/`), `ts/docs` — CLI-спеки. Спеки
platform/registry и platform/dispatcher возвращены в «черновик» до
пересмотра дробления; снятые golden остаются эталоном CLI-поверхности.
...
- **config** — рантайм-конфиг (`~/.config/mpu/.env` + локальный кэш).
  Сторона: расщепляется (секреты/серверное — `.env` брокера; локальные
  предпочтения — CLI). Спека: черновик.
- **invoke-log** — журнал вызовов: каждая команда пишет одну запись.
  Сторона: брокер (центральный аудит операций; `mpu log` — операция
  чтения; локальные команды без брокера в журнал не попадают).
  Спека: черновик.
...
- **dispatcher** — таблица «команда → Go | Python», вызов Python-ветки.
  Сторона: распадается — делегация Python → операция брокера,
  маршрутизация → реестр CLI. Спека: черновик (пересмотр, см. шапку;
  golden сняты 2026-07-29 и остаются эталоном).
- **registry** — реестр команд: `help`, `version`, discovery.
  Сторона: CLI. Спека: черновик (пересмотр, см. шапку; golden сняты
  2026-07-29 и остаются эталоном).
