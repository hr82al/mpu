...
def _patch_roles_cache(monkeypatch: pytest.MonkeyPatch, rows: list[tuple[int, str]]) -> None:
...
def _freeze_now(monkeypatch: pytest.MonkeyPatch, moment: datetime.datetime) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (datetime.datetime(2026, 7, 20, 21, 30, tzinfo=_UTC), "2026-07-21"),
        # 20:30 UTC = 23:30 МСК того же дня
        (datetime.datetime(2026, 7, 20, 20, 30, tzinfo=_UTC), "2026-07-20"),
    ],
)
...
def test_elapsed_minutes_rounds_up(seconds: int, expected: int) -> None:
    start = datetime.datetime(2026, 7, 20, 9, 0, tzinfo=_UTC)
...
def test_timer_window_without_time_sends_only_finish() -> None:
    now = datetime.datetime(2026, 7, 20, 12, 34, 56, tzinfo=_UTC)
...
def test_timer_window_with_time_keeps_start_like_web_ui() -> None:
    ...
    now = datetime.datetime(2026, 7, 20, 15, 53, 30, tzinfo=_UTC)
...
def test_timer_window_clamps_future_finish_to_now() -> None:
    ...
    now = datetime.datetime(2026, 7, 20, 9, 10, 0, tzinfo=_UTC)
...
def test_timer_window_preserves_source_timezone() -> None:
    ...
    now = datetime.datetime(2026, 7, 20, 9, 0, 0, tzinfo=_UTC)
...
def test_timer_window_milliseconds_always_zero() -> None:
    ...
    now = datetime.datetime(2026, 7, 20, 9, 30, 0, 500_000, tzinfo=_UTC)
...
def test_summarise_logs_filters_and_totals() -> None:
    logs = [
        _log(log_id=1, user_id=42, minutes=60, for_date="2026-07-19"),
        _log(log_id=2, user_id=99, minutes=30, for_date="2026-07-20"),  # чужая
        _log(log_id=3, user_id=42, minutes=15, for_date="2026-07-20", role_id=12057),
    ]
...
def test_resolve_role_by_name_substring_and_id(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_roles_cache(monkeypatch, _ROLES)
...
def test_resolve_role_env_override(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_roles_cache(monkeypatch, _ROLES)
    _install_env(monkeypatch, {"KITEN_TIME_ROLE": "Код-ревью"})
...
def test_resolve_role_bad_env_does_not_fall_back(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _patch_roles_cache(monkeypatch, _ROLES)
    _install_env(monkeypatch, {"KITEN_TIME_ROLE": "нет такой"})
...
def test_resolve_role_numeric_works_without_cache(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _patch_roles_cache(monkeypatch, [])
...
def test_time_add_posts_minutes_and_msk_today(
    ...
    fake = FakeKaitenClient(user=_user())
    _install_client(monkeypatch, fake)
...
def test_time_add_api_error_exits_1(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    ...
    _install_env(monkeypatch, {})
    _freeze_today(monkeypatch)
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), fail={"add_time_log"}))
...
def test_time_ls_mine_only_by_default(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    logs = [_log(log_id=1, user_id=42), _log(log_id=2, user_id=99, user="Коллега")]
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), time_logs=logs))
...
def test_time_ls_output_carries_no_avatars(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), time_logs=[_log()]))
    res = runner.invoke(app, ["time", "ls", "100", "--json"])
    assert res.exit_code == 0, res.stderr
...
def test_time_ls_date_window(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    logs = [
        _log(log_id=1, for_date="2026-07-18"),
        _log(log_id=2, for_date="2026-07-20"),
    ]
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), time_logs=logs))
...
def test_time_edit_sends_only_given_axes(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _patch_roles_cache(monkeypatch, _ROLES)
    fake = FakeKaitenClient(user=_user(), time_logs=[_log(log_id=7)])
    _install_client(monkeypatch, fake)
...
def test_time_edit_missing_log_hints_ls(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), time_logs=[_log(log_id=7)]))
...
def test_time_edit_foreign_log_blocked_and_forced(
    ...
    fake = FakeKaitenClient(user=_user(), time_logs=[_log(log_id=7, user_id=99)])
    _install_client(monkeypatch, fake)
...
def test_time_rm_prints_deleted_record(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    fake = FakeKaitenClient(user=_user(), time_logs=[_log(log_id=7, comment="разбор жалобы")])
    _install_client(monkeypatch, fake)
...
def test_time_start_records_hint_with_role(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _patch_roles_cache(monkeypatch, _ROLES)
    _install_env(monkeypatch, {})
    fake = FakeKaitenClient(user=_user(), details=[_detail(timer=None)])
    _install_client(monkeypatch, fake)
...
def test_time_start_when_already_running_here(
    ...
    _patch_roles_cache(monkeypatch, _ROLES)
    _install_env(monkeypatch, {})
    fake = FakeKaitenClient(user=_user(), details=[_detail(timer=_timer())])
    _install_client(monkeypatch, fake)
...
def test_time_start_when_server_says_already_created(
    ...
    _patch_roles_cache(monkeypatch, _ROLES)
    _install_env(monkeypatch, {})
    fake = FakeKaitenClient(user=_user(), details=[_detail(timer=None)], timer_already_running=True)
    _install_client(monkeypatch, fake)
    res = runner.invoke(app, ["time", "start", "100"])
    assert res.exit_code == 1
...
def test_time_start_blocked_by_verified_timer_elsewhere(
    ...
    fake = FakeKaitenClient(
        user=_user(),
        details=[_detail(card_id=100, timer=None), _detail(card_id=555, timer=_timer())],
    )
    _install_client(monkeypatch, fake)
    res = runner.invoke(app, ["time", "start", "100"])
    assert res.exit_code == 1
...
def test_time_start_clears_stale_hint_and_proceeds(
    ...
    fake = FakeKaitenClient(user=_user(), details=[_detail(card_id=100, timer=None)])
    _install_client(monkeypatch, fake)
    res = runner.invoke(app, ["time", "start", "100"])
    assert res.exit_code == 0, res.stderr
...
def test_time_status_running(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _freeze_now(monkeypatch, datetime.datetime(2026, 7, 20, 10, 15, tzinfo=_UTC))
    detail = _detail(timer=_timer(comment="правлю"), time_spent_sum=255)
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), details=[detail]))
...
def test_time_status_idle_is_not_an_error(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _install_client(
        monkeypatch, FakeKaitenClient(user=_user(), details=[_detail(timer=None, time_spent_sum=0)])
    )
    res = runner.invoke(app, ["time", "status", "100"])
...
def test_time_status_json(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _freeze_now(monkeypatch, datetime.datetime(2026, 7, 20, 9, 30, tzinfo=_UTC))
    detail = _detail(timer=_timer(), time_spent_sum=60)
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), details=[detail]))
...
def test_time_status_without_selector_uses_hint(
    ...
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), details=[_detail(timer=_timer())]))
...
def test_time_stop_without_time_sends_only_finish(
    ...
    _patch_roles_cache(monkeypatch, _ROLES)
    _install_env(monkeypatch, {})
    _freeze_now(monkeypatch, datetime.datetime(2026, 7, 20, 10, 0, tzinfo=_UTC))
    created = _log(log_id=4471, minutes=60)
    fake = FakeKaitenClient(user=_user(), details=[_detail(timer=_timer())], time_logs=[created])
    _install_client(monkeypatch, fake)
...
def test_time_stop_with_time_shifts_timestamps(
    ...
    _patch_roles_cache(monkeypatch, _ROLES)
    _install_env(monkeypatch, {})
    _freeze_now(monkeypatch, datetime.datetime(2026, 7, 20, 13, 12, tzinfo=_UTC))
    created = _log(log_id=4471, minutes=75)
    fake = FakeKaitenClient(user=_user(), details=[_detail(timer=_timer())], time_logs=[created])
    _install_client(monkeypatch, fake)
...
def test_time_stop_carries_start_comment(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    ...
    fake = FakeKaitenClient(
        user=_user(),
        details=[_detail(timer=_timer(comment="работа по карточке"))],
        time_logs=[_log(log_id=4471)],
    )
    _install_client(monkeypatch, fake)
    res = runner.invoke(app, ["time", "stop", "100"])
    assert res.exit_code == 0, res.stderr
...
def test_time_stop_reads_back_created_log(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    ...
    _install_env(monkeypatch, {})
    _freeze_now(monkeypatch, datetime.datetime(2026, 7, 20, 10, 0, tzinfo=_UTC))
    created = _log(log_id=4471, minutes=99, for_date="2026-07-19", comment="с сервера")
    fake = FakeKaitenClient(user=_user(), details=[_detail(timer=_timer())], time_logs=[created])
    _install_client(monkeypatch, fake)
    res = runner.invoke(app, ["time", "stop", "100"])
    assert res.exit_code == 0, res.stderr
...
def test_time_stop_warns_on_utc_date_shift(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    ...
    _patch_roles_cache(monkeypatch, _ROLES)
    _install_env(monkeypatch, {})
    _freeze_now(monkeypatch, datetime.datetime(2026, 7, 20, 21, 30, tzinfo=_UTC))
    created = _log(log_id=4471, for_date="2026-07-20")
    fake = FakeKaitenClient(user=_user(), details=[_detail(timer=_timer())], time_logs=[created])
    _install_client(monkeypatch, fake)
    res = runner.invoke(app, ["time", "stop", "100"])
    assert res.exit_code == 0, res.stderr
...
def test_time_stop_uses_role_from_start_hint(
    ...
    fake = FakeKaitenClient(
        user=_user(), details=[_detail(timer=_timer())], time_logs=[_log(log_id=4471)]
    )
    _install_client(monkeypatch, fake)
    res = runner.invoke(app, ["time", "stop", "100"])
    assert res.exit_code == 0, res.stderr
...
def test_time_ls_summary_fans_out_over_cards(
    ...
    logs = [
        _log(log_id=1, card_id=100, minutes=60, for_date="2026-07-20"),
        _log(log_id=2, card_id=200, minutes=30, for_date="2026-07-20"),
    ]
    fake = FakeKaitenClient(
        user=_user(),
        cards=[_card(card_id=100), _card(card_id=200)],
        time_logs=logs,
    )
    _install_client(monkeypatch, fake)
...
def test_time_ls_summary_includes_hinted_cards(
    ...
    fake = FakeKaitenClient(
        user=_user(), cards=[], time_logs=[_log(log_id=9, card_id=300, for_date="2026-07-20")]
    )
    _install_client(monkeypatch, fake)
    res = runner.invoke(app, ["time", "ls", "--date-from", "2026-07-20", "--json"])
    assert res.exit_code == 0, res.stderr
...
def test_time_ls_summary_empty_reports_scanned(
    ...
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), cards=[_card(card_id=100)]))
...
def test_install_client_covers_every_command_module() -> None:
    # Забытый в `_COMMAND_MODULES` модуль = тесты, молча ходящие в сеть.
    import pkgutil
    ...
    covered = {mod.__name__.rsplit(".", 1)[-1] for mod in _COMMAND_MODULES}
...
def test_time_ls_summary_table_output(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    logs = [
        _log(log_id=1, card_id=100, minutes=60, for_date="2026-07-20", comment="первое"),
        _log(log_id=2, card_id=200, minutes=30, for_date="2026-07-19", user="Коллега", user_id=99),
    ]
    fake = FakeKaitenClient(
        user=_user(), cards=[_card(card_id=100), _card(card_id=200)], time_logs=logs
    )
    _install_client(monkeypatch, fake)
...
def test_time_ls_summary_survives_unreadable_card(
    ...
    fake = FakeKaitenClient(user=_user(), cards=[_card(card_id=100)], fail={"list_time_logs"})
    _install_client(monkeypatch, fake)
    res = runner.invoke(app, ["time", "ls", "--date-from", "2026-07-20"])
    assert res.exit_code == 0, res.stderr
...
def test_time_ls_summary_survives_list_cards_failure(
    ...
    fake = FakeKaitenClient(
        user=_user(),
        time_logs=[_log(log_id=9, card_id=300, for_date="2026-07-20")],
        fail={"list_cards"},
    )
    _install_client(monkeypatch, fake)
    res = runner.invoke(app, ["time", "ls", "--date-from", "2026-07-20", "--json"])
...
def test_time_ls_api_error_exits_1(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), fail={"list_time_logs"}))
    res = runner.invoke(app, ["time", "ls", "100"])
    assert res.exit_code == 1
...
def test_time_ls_role_filter(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _patch_roles_cache(monkeypatch, _ROLES)
    logs = [_log(log_id=1, role_id=12058), _log(log_id=2, role_id=12057)]
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), time_logs=logs))
...
def test_time_edit_api_error_exits_1(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _install_client(
        monkeypatch,
        FakeKaitenClient(user=_user(), time_logs=[_log(log_id=7)], fail={"update_time_log"}),
    )
...
def test_time_rm_api_error_exits_1(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _install_client(
        monkeypatch,
        FakeKaitenClient(user=_user(), time_logs=[_log(log_id=7)], fail={"delete_time_log"}),
    )
    res = runner.invoke(app, ["time", "rm", "100", "7"])
    assert res.exit_code == 1
...
def test_time_start_api_error_exits_1(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    ...
    _install_client(
        monkeypatch,
        FakeKaitenClient(user=_user(), details=[_detail(timer=None)], fail={"start_timer"}),
    )
    res = runner.invoke(app, ["time", "start", "100"])
    assert res.exit_code == 1
...
def test_time_status_api_error_exits_1(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    _install_client(monkeypatch, FakeKaitenClient(user=_user(), fail={"get_card"}))
    res = runner.invoke(app, ["time", "status", "100"])
    assert res.exit_code == 1
...
def test_time_stop_api_error_exits_1(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    ...
    _install_client(
        monkeypatch,
        FakeKaitenClient(user=_user(), details=[_detail(timer=_timer())], fail={"stop_timer"}),
    )
    res = runner.invoke(app, ["time", "stop", "100"])
    assert res.exit_code == 1
...
def test_time_discard_api_error_exits_1(monkeypatch: pytest.MonkeyPatch, db_path: Path) -> None:
    ...
    _install_client(
        monkeypatch,
        FakeKaitenClient(user=_user(), details=[_detail(timer=_timer())], fail={"discard_timer"}),
    )
    res = runner.invoke(app, ["time", "discard", "100"])
    assert res.exit_code == 1
...
def test_time_start_ignores_unreadable_hinted_card(
    ...
    fake = _PickyFake(user=_user(), details=[_detail(card_id=100, timer=None)])
    _install_client(monkeypatch, fake)
    res = runner.invoke(app, ["time", "start", "100"])
    assert res.exit_code == 0, res.stderr
    assert fake.timers_started
