...
| `mpu kiten` | `commands/kiten.py` | Kaiten (btlz.kaiten.ru): `card <id\|url>` — карточка (`--md`/`--json`); `ls` — мои карточки + фильтры space/board/lane/column/state/дата; `comment <card> -m\|-F` — коммент от своего имени; `move <card> --lane\|--column\|--board` / `ready <card>` (→«Готово») / `review <card>` (→ревью) — перемещение (точный матч названия колонки в приоритете; каждое пишется в журнал `kaiten_card_moves`; уже в целевой колонке → авто релог-bump влево→обратно, т.к. Kaiten не логирует ход в ту же колонку); `field set\|ls\|update\|rm <card> <mr\|hypothesis\|done\|result>` — значение в кастомное поле карточки + история в sqlite (несколько MR на карточку); `spaces`/`boards`/`lanes`/`columns` — справочник + кэш; `whoami`; ENV `KITEN_API_KEY`, `KITEN_LS_*`, `KITEN_READY_COLUMN`/`KITEN_REVIEW_COLUMN`/`KITEN_STATUS_EMOJI` |
