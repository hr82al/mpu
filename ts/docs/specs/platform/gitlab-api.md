...
Статус: реализовано — сверено 2026-09-10 по потребителю `mpu mr view --help` (возможность используется работающей командой; контракт построчно не сверялся). Ранее: к реализации (заморожена 2026-08-27 вместе с `mr-read.md`; формы
ответов сняты живьём на MR, мутирующая часть контракта живым прогоном не
подтверждена — см. «Golden-примеры» в `mr-write.md`)
