...
Статус: реализовано — сверено 2026-09-10 по `mpu wb-jobs --help` (команда в справке; поверхность флагов построчно не сверялась). Ранее: к реализации (заморожена 2026-08-17; уточнена по приёмке
...
`mpu ssh target: dev:N cmd: '<inner>'` (с порции 159; до неё — `mpu ssh dev:N -- <inner>`): до dev-ноды ходит соседняя команда, и второй
...
sl-сервера: ``dev:N`` (например dev:1)`, exit 2. `--server` вместе с
`dev:N` — тоже ошибка ввода: сервер на этой ветке выбран селектором, и
второй источник адреса означал бы, что названное оператором значение
никуда не поехало.
...
У `process` парой с рабочей версией сверены **только печатные формы**:
дефолтный режим — пересчёт витрин на живых данных, и живьём он не
гонялся ни на одной ветке, включая `dev:N`. Всё, что спека утверждает
про исполнение (стриминг вывода, наследование кода выхода, транспорт
ветки `dev`), выведено из общей машинерии семейства и проверено
тестами, а не прогоном.
...
- **preserve** — `--no-is-active` у `ss-datasets` не эмитит ничего:
  ...
  Вердикт изменён 2026-08-31 на **fix**: бэкенд принимает булево `false`
  (разбор через `JSON.parse`, проверка `is_active === true || is_active
  === false`), значит обёртка обязана эмитить `--is-active false`.
  Прежний вердикт **preserve** держался на непроверенном вопросе — см.
  «Открытые вопросы».
- **improve** — отказ `dev:N` без `--client-id`. Рабочая версия
  отвечает общим текстом про резолв селектора, здесь — `mpu process:
  dev-селектор требует --client-id: кандидатов на dev-ноде нет`. Обе
  формы дают exit 2; наша называет причину, а не следствие.
- **fix** — `--server` вместе с `dev:N`. Рабочая версия печатает
  `mpu ssh dev:1 -- …`, молча проглотив названный сервер (проверено
  печатью). Здесь это ошибка ввода: сервер, названный оператором и
  никуда не поехавший, не должен исчезать без следа.
...
**Закрыт 2026-08-31:** выключение принимается формой `--is-active false`.
Ответ снят не догадкой, а чтением бэкенда, куда обёртка целится:

- разбор аргументов метода прогоняет каждое значение через `JSON.parse`,
  поэтому `false` доезжает **булевым**, а не строкой;
- проверка параметров метода пропускает значение при
  `is_active === true || is_active === false` — то есть `false` принимается
  наравне с `true`, а не отбрасывается как пустое;
- запись в модель делает то же самое: булево `false` сохраняется.

Значит выключить признак командой можно, и мешает этому только сама
обёртка. Вердикт отклонения меняется с **preserve** на **fix**: при
`--no-is-active` обёртка обязана эмитить `--is-active false`, а не молчать.
Прежняя формулировка («как метод принимает выключение — не проверено, а
гадать в обёртке нельзя») была верной ровно до тех пор, пока никто не
посмотрел.
