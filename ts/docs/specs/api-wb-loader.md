...
Статус: реализовано — сверено 2026-09-10 по `mpu api wb-loader-status --help` (команда в справке; поверхность флагов построчно не сверялась). Ранее: черновик

## Назначение

Шесть команд управления загрузчиками wb-loader-app через admin-прокси
...
### mpu api wb-loader-config

`wb-loader-config SELECTOR LOADER [--sid SID] [--client-id ID]
[--enable | --disable | --reset] [--print/-p]`

Без флагов — чтение: `GET /admin/wb-loader/loaders/<sid>/<loader>/v1/config`,
четыре части ответа — действующие параметры (база плюс дельта кабинета),
базовые, сырая дельта этого кабинета и перечень полей, редактируемых per-sid.

Мутации — это и есть включение/выключение конкретного загрузчика на
конкретном кабинете. **Три флага взаимоисключающи**: два вместе — код 2 до
сети, а не «последний выигрывает».

Формы, снятые с объекта (метод у всех трёх — **`POST`**, не `PATCH`):

| флаг | путь | тело |
|---|---|---|
| `--enable` | `…/v1/config` | `{"enabled": true}` |
| `--disable` | `…/v1/config` | `{"enabled": false}` |
| `--reset` | `…/v1/config/reset` | **тела нет** |

Два различия, которые легко потерять:

1. **`--reset` — другой путь, а не другое тело.** Он снимает дельту кабинета
   целиком и возвращает базовую конфигурацию; тело не отправляется вовсе.
2. **Тело включения/выключения — частичная дельта, а не полная замена.**
   Шлётся ровно `{"enabled": …}`, остальные поля кабинета не затираются.
   Полная замена стёрла бы чужие настройки молча.

### mpu api wb-loader-load

`wb-loader-load SELECTOR LOADER [--sid SID] [--client-id ID] [--print/-p]`

`POST …/v1/load` — форс-прогон: отложенная задача выполняется немедленно, не
дожидаясь своего цикла. Читающий аналог — `wb-loader-status`.

### mpu api wb-loader-reset

`wb-loader-reset SELECTOR LOADER [--sid SID] [--client-id ID]
[--state JSON | --from YYYY-MM-DD] [--and-load] [--print/-p]`

`POST …/v1/reset` — сброс состояния и перезапуск: ближайший прогон
пересчитывает окно. Без опций тело пустое.

Окно дозагрузки задаётся **одним из двух взаимоисключающих** способов:

- `--state` — частичное состояние как есть, для любого загрузчика;
- `--from` — только для загрузчиков, идущих вперёд по дате: собирает состояние
  с датой **на день раньше** указанной, чтобы ближайший прогон пошёл именно с
  неё.

Два вместе — код 2 до сети.

**Верхняя граница окна у таких загрузчиков всегда «вчера»** — участок точно
не задаётся, свежие дни перезаливаются. Это свойство загрузчика, не команды;
в справке сказать прямо, иначе оператор ждёт точного окна.

`--and-load` следом дёргает форс-прогон — типовое восстановление одним
вызовом. Отказ второго шага не отменяет первого: сброс уже произошёл, и
сообщение обязано это сказать (тот же класс, что частичная запись в
`sheet set`).
