...
Статус: к реализации — не реализовано полностью (2026-09-10): режим клиента наследует дефекты
шагов 1–2 `specs/copy-client.md` (версия утилит, роль клиента) — требования там, раздел «Версия
утилит и роль клиента — требования 2026-09-10». Ранее: реализовано — сверено 2026-09-10 по справке
...
Режим клиента — шаги 1–4 `specs/copy-client.md` вместе с её требованиями 2026-09-10 к версии
утилит и роли клиента (дамп/восстановление
...
- **fix** — тексты отказов неполной конфигурации приходят от слоя
  env-файла дословно (`environment variable DEV_WORKSPACES_USER is not
  set. Add it to <путь> or export in shell.`), а не своими
  формулировками вида `dev workspaces creds: задайте …`. Одна
  формулировка на все команды лучше своей копии в каждой: слой называет
  и ключ, и путь файла, то есть говорит больше. Решение 2026-08-28, тот
  же довод, что у `WB_PLUS_WEB_APP_URL` в `sheet.md`.

## Открытые вопросы

нет

Расширение: режим `--ozon` — перенос кабинета dev-контура вертикали Ozon (ClickHouse) с доливкой хвоста — `specs/copy-dev-ozon.md` (только спецификация).
