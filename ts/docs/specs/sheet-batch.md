...
- **fix** — `inheritFromBefore` при вставке в нулевой индекс. Оригинал шлёт
  `true` независимо от индекса, и Google отвергает всю пачку целиком:
  `Invalid requests[0].insertDimension: range.startIndex must not be 0 if
  inheritFromBefore is true` (замер 2026-08-28 на служебной таблице). У нас
  `true` везде, кроме `startIndex = 0`, где `false`: смысл «наследовать формат
  слева» сохраняется, а на левом краю наследовать неоткуда.
