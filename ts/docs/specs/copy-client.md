...
Статус: к реализации — не реализовано полностью (2026-09-10): команда в справке есть, но шаг 1
падает на `pg_restore` новее приёмника и не доходит до шагов 2–4, а шаг 2 (роль клиента) не выполняется
вовсе — живой случай copy-dev 54, раздел «Версия утилит и роль клиента — требования 2026-09-10». Ранее:
реализовано — сверено 2026-09-10 по справке; к реализации (заморожена 2026-08-28)
...
2. **Роль клиента на локальном sl-1 — повторяет бутстрап sl-back.** Под одной транзакцией:
   если в `pg_roles` нет `client_<id>` — `CREATE ROLE client_<id> LOGIN`; затем всегда
   `ALTER ROLE client_<id> WITH LOGIN PASSWORD '<PG_CLIENT_USER_PASSWORD>'`,
   `ALTER ROLE client_<id> SET search_path TO schema_<id>, shared`,
   `ALTER SCHEMA schema_<id> OWNER TO client_<id>`, `GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA`
   и `… ON ALL SEQUENCES IN SCHEMA schema_<id>` этой роли, `ALTER DEFAULT PRIVILEGES IN SCHEMA
   schema_<id> GRANT ALL PRIVILEGES ON TABLES` и `… ON SEQUENCES` ей же. Почему сама команда, а не
   sl-back: его бутстрап роли идёт только когда схемы ещё нет, а после шага 1 схема уже есть — роль
   без этого шага не появится никогда (замер 2026-09-10: `password authentication failed for user
   "client_54"`, миграции клиента падают на первом же шаге). Пароль — тот же, под которым ходит
   локальный инстанс sl-1: после шага команда проверяет вход ролью `client_<id>` в локальный sl-1;
   не входит → exit 1, `роль client_<id>: вход с PG_CLIENT_USER_PASSWORD не проходит — пароль не
   совпадает с инстансом sl-1`. Переменная не задана → exit 2 до шага 1 (роль без пароля инстансу
   бесполезна).
...
Следствие снятия связок: **воркспейс, у которого снята последняя связка,
остаётся в базе пустым** — без кабинетов и подписок. Команда сознательно не
трогает чужие сущности сверх самой связки: удалять чужое пространство по
косвенному признаку «в нём ничего не осталось» опаснее, чем оставить его
пустым — опустеть оно могло и не из-за нас.
...
## Версия утилит и роль клиента — требования 2026-09-10

Живой случай: `mpu copy-dev 54` восстановил `schema_54` (207 таблиц), завершился `pg_restore … failed
(exit 1)` из-за `SET transaction_timeout` и не выполнил шаги 2–4; инстанс sl-1 отвечал «Client not
found», оболочка ждала статусы подписок 7,5 с на каждое открытие экрана. Доведено руками: строка
`clients`, `clients_wb_cabinets`, роль по шагу 2.

- **Утилиты — мажорной версии приёмника.** До шага 1 команда читает мажор локального sl-1
  (`SHOW server_version_num`) и мажор `pg_dump`/`pg_restore` в `PATH` (`--version`). Совпали — как
  сейчас. Утилита новее приёмника — `pg_dump` и `pg_restore` запускаются в контейнере
  `postgres:<мажор приёмника>-alpine` (`docker run --rm --network host`, каталог временных файлов
  примонтирован по тому же пути — приём проверен 2026-08-28, раздел «Известные ловушки окружения»).
  Docker недоступен → exit 2 до дампа: `pg_dump <v> новее приёмника <v>, контейнер утилит
  недоступен: <причина>`. Контракт «ненулевой код `pg_restore` — отказ» не меняется: при совпавших
  версиях его просто не бывает.
- **Шаги 2–4 выполняются всегда, когда шаг 1 прошёл.** Отказ шага 1 останавливает команду (как
  сейчас), но сообщение называет, что не сделано: `шаги 2–4 не выполнены: роль, public-строки и
  токен-строки клиента не перенесены`.
- **Итог — проверкой, а не счётчиками.** Последней строкой stderr перед summary — вход ролью
  `client_<id>` удался и строка `clients` с `server = 'sl-1'` на месте; иначе exit 1.

**Пункты чек-листа проектирования для этих требований** (что каждый заставил изменить):

| Пункт | Что изменено |
| --- | --- |
| 1 · примитивы | примитив один — «запустить утилиту Postgres с аргументами и паролем в окружении»; дамп и восстановление — его вызовы. Выбор, где запускать (хост или контейнер нужной версии), делается один раз до шага 1, а не у каждого вызова |
| 2 · особые случаи → получатели | «утилита новее приёмника» — не ветка у каждого вызова, а вторая реализация того же примитива (запуск в контейнере `postgres:<мажор>-alpine`); выбирается один раз по сравнению мажоров. Роль «уже есть / нет» — идемпотентный шаг, а не ветвление: `CREATE ROLE` только при отсутствии, остальные операторы всегда (как бутстрап sl-back) |
| 3 · null | `PG_CLIENT_USER_PASSWORD` не задан — отказ до шага 1 (exit 2), а не роль без пароля и не пропуск шага |
| 4 · кто решает | реализацию запуска выбирает оркестратор копии по двум числам (мажор приёмника, мажор утилиты); шаги 2–4 ничего о версии не знают. Итог «копия готова» решает проверка входа ролью, а не счётчики строк |
| 5 · память приватна | ничего |
| 6 · голые данные → протокол | версия — число-мажор, разобранное один раз из `server_version_num` и из `--version`; строка версии дальше не ходит. Отказы — текстами контракта, не текстом чужой ошибки |

Golden: снимаются при реализации на стенде (клиент 54 или синтетический) — прогон при совпавших
версиях, прогон с утилитой новее приёмника (контейнер), отказ входа ролью с неверным паролем.
...
- **Долгий прогон через MCP (2026-09-24, клиент 54 с sl-9).** Дамп схемы шёл больше 45 минут; строки
  `… pg_dump идёт Ns` уходят в stderr, но MCP-клиент прогрессом их не видит и через 300 с тишины
  обрывает вызов (`sent no response or progress for 300s`). Воркер при этом копирование НЕ прерывает:
  повторный вызов запустил второй `pg_dump` той же схемы с прода параллельно первому (второй пришлось
  гасить руками). Нужно решить: (а) слать MCP progress-уведомления из тех же тиков «идёт Ns»;
  (б) при старте — отказ, если копия того же клиента уже идёт (замок по client_id), с текстом «уже
  копируется, pid N, идёт Ns».
...
Расширение: окно по дате и доливка хвоста — `specs/copy-client-window.md` (только спецификация).
