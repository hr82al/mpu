...
**Сброс кэша sw-back на стенде печатает отказ авторизации, но работает**
(замер 2026-08-28). У контейнера `redis-dev` стенда пароль не настроен вовсе,
поэтому `redis-cli -a some-redis-password` пишет в stderr `AUTH failed: ERR
AUTH called without any password configured`, после чего **выполняет команду**
и завершается кодом 0 (проверено отдельным `DBSIZE` — отвечает). Строка в
выводе выглядит как провал шага и им не является; на проде, где пароль задан,
её нет.
