...
**Отказ самого redis не виден по коду возврата** (замер 2026-08-28). Граница
проходит так: отсутствующий контейнер ловит `docker` — код 1; ошибка redis
(неверная арность, неизвестная команда) даёт **код 0**, а текст `ERR …`
печатается в **stdout**, не в stderr (проверено `cat -A`: второй поток пуст).
Поэтому шаг разбирает stdout и считает отказом строку, начинающуюся с `ERR`
(и форму `(error) ERR …`), а stderr не читает вовсе — иначе безобидное
предупреждение AUTH выше стало бы отказом.
