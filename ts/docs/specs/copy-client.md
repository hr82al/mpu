...
**Проводка входа sw-front — шаг best-effort и на стенде падает.** Наблюдаемо:
`WARN проводка sw-front не удалась (column "is_active" of relation
"workspaces" does not exist)`. Схема локальной БД воркспейсов отличается от
ожидаемой шагом; копия клиента при этом цела, отказом это не является. Правка
формы вставки — отдельная работа, не эта команда.
...
Право нельзя задать одной строкой `$TMPDIR`: при незаданной переменной элемент
списка пуст, и Deno отвечает `Empty values are not allowed` — бинарь перестаёт
запускаться у всех, у кого переменная не выставлена. Допустимые формы — явные
