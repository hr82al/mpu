...
- ~~Повторный прогон при существующей роли `client_<id>`~~ — снято 2026-08-28:
  команда роль **не создаёт вовсе**. После копирования клиента 5175 на стенде
  роли `client_5175` нет (есть только `client_54` от прежних потоков), а
...
- Повторный прогон при уже существующей роли `client_<id>` с другим
  search_path: контракт — `ALTER ROLE … SET search_path` перезаписывает
  значение; отдельным тестом оригинала не закреплено — проверить при
  снятии golden.
