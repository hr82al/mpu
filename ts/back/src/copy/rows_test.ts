...
Deno.test("значение параметра: json сериализуется, остальное — как есть", async (t) => {
  ...
  await t.step("скаляр в json тоже сериализуется", () => {
    ...
    assertEquals(paramOf("привет", JSONB_OID), '"привет"');
    assertEquals(paramOf(42, JSONB_OID), "42");
    assertEquals(paramOf(true, JSONB_OID), "true");
  });

  await t.step("строка с текстом JSON внутри не меняет тип молча", () => {
...
Deno.test("круг json: что пришло из колонки, то и уходит обратно", () => {
  ...
  for (
    ...
    assertEquals(paramOf(parse?.(raw), JSONB_OID), raw, raw);
  }
});
