...
async function runD2Miro(
  ...
  requireSomethingToDraw(paths, plan, args["skip-render"]);
...
/**
 * Рисовать нечего — отказ до всякого обращения к службе, и доска не
 * трогается вовсе.
 *
 * Прежде такой прогон создавал на доске пустой фрейм и возвращал ноль
 * (замер спецификатора: устаревший SVG рядом с `.d2`, план
 * `0 shapes, 0 markdown`, фрейм создан). Успех при несделанной работе
 * — то же, что и код 0 при полном провале отрисовки, только раньше по
 * времени.
 *
 * Код 2, а не 1: работать не над чем — это про вход, а не про службу
 * (её отказ по-прежнему код 1). Отказ одинаков и при `--dry-run`:
 * план обязан совпадать с рендером (отклонение-fix спеки), а показ
 * пустого списка вместо отказа делал бы предпросмотр враньём — он
 * говорил бы «всё в порядке, делать нечего» там, где вход не тот.
 */
function requireSomethingToDraw(
  paths: Paths,
  plan: Plan,
  skipRender: boolean,
): void {
  if (plan.shapes.length + plan.markdown.length > 0) return;
  // Названы оба источника: шейпы приходят из SVG, markdown-блоки — из
  // самого `.d2`. Свалить пустоту на один файл значило бы отправить
  // оператора чинить не тот.
  throw new UsageError(
    `в плане нет ни одного объекта: ни шейпов в ${paths.svgName}, ни ` +
      `markdown-блоков в ${paths.name}`,
    {
...
Рисует .d2 на доске Miro редактируемым фреймом: шейпы и тексты по
...
2 — ошибки ввода и конфигурации, включая пустой план (ни шейпов, ни
markdown-блоков: доска не трогается вовсе); 1 — отказ Miro API, сбой
