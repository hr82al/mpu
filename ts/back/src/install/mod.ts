...
/**
 * Путь установленной программы: `mpu build` заменяет по нему файл, а
 * описание службы MCP ставит его в `ExecStart` (`docs/specs/build.md`,
 * `docs/specs/mcp-service.md`). Разъехавшись, эти двое дали бы службу,
 * приколоченную не к тому файлу, который заменяет сборка.
 *
 * Третий, кто знает тот же путь, — значение `-o` задачи `build` в
 * `deno.jsonc`: там он записан отдельно, потому что задачу читает
 * `deno task`, а не наш код. Совпадение стережёт тест
 * (`mod_test.ts`, «путь установки записан одинаково»), а не эта
 * функция: подставлять её значение в `-o` нельзя — `scripts/smoke.ts`
 * собирает бинарь во временный каталог тем же разбором.
 *
 * @param home домашний каталог пользователя
 */
export function installedBinPath(home: string): string {
  return `${home}/.local/bin/mpu`;
}
