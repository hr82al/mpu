...
Режим выбирает набор ключей:

  range: + value:   одна ячейка
  from: ФАЙЛ        пакет из файла
  без них           JSON из потока (stdin)

В JSON-режиме диапазоны — в самом JSON, цель — spreadsheet:.
