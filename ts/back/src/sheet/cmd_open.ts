...
export const sheetOpenCommand = defineCommand({
  ...
  usage: "mpu sheet open [tab: ЛИСТ] [spreadsheet: SS]",
  help: `Звать, когда таблицу или её вкладку надо показать человеку в
браузере, или нужна сама ссылка.

Печатает ссылку и передаёт её открывателю (xdg-open, затем
open). Печать от запуска не зависит: ссылка нужна и тогда, когда
открывать нечем — на сервере без графики, в пайпе, в логе.

Без tab: открывается таблица целиком, и кэш метаданных при этом не
трогается. С tab: спрашивается перечень вкладок — он и даёт числовой
...
и отсутствующий лист (с перечнем доступных).`,
  examples: [
    "mpu sheet open spreadsheet: otchet",
    "mpu sheet open tab: Сводка spreadsheet: otchet",
  ],
