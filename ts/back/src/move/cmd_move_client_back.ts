...
export const moveClientBackCommand = defineCommand({
  ...
  usage: "mpu move-client-back [target: СЕЛЕКТОР | ls | rm target: СЕЛЕКТОР]",
  help: `Звать, когда перенос клиента надо откатить или посмотреть журнал
переносов.

Возвращает клиента туда, откуда его перенесла mpu move-client:
...
Без ключей или сообщением ls печатает записанные ходы: клиент, маршрут и
время записи, новые сверху.

rm target: удаляет запись, ничего не перенося. Повторный rm — тоже
...
запись, rm без селектора, резолв селектора; иначе — код createJob.`,
  examples: [
    "mpu move-client-back target: 1234",
    "mpu move-client-back ls",
