...
Deno.test("ввод: поле кадра доходит до команды, без поля — пусто", () =>
  withBack(async (back) => {
    ...
    assertEquals(await line(back, "/line", ["confirm", "-y"]), [
      { err: "\n" },
      { out: "" },
      { exit: 0 },
    ]);
...
Deno.test("ввод: пустая строка — это ввод", () =>
  withBack(async (back) => {
    assertEquals(await lineWith(back, ["confirm", "-y"], { stdin: "" }), [
      { err: "\n" },
      { out: "" },
      { exit: 0 },
    ]);
...
Deno.test("ввод больше предела: отказ до исполнения, по обеим дверям", async (t) => {
  ...
  await t.step("WebSocket", () =>
    withBack(async (back) => {
      assertEquals(await lineWith(back, ["confirm", "-y"], { stdin: big }), [
        ...refusal,
      ]);
...
Deno.test("вопрос по номеру: исполнение видит тот же контекст", () =>
  withBack(async (back) => {
    ...
    assertEquals(asked[0].ask, "выполнить mpu confirm -y? [y/N] ");
