...
export const logCommand = defineCommand({
  ...
  usage:
    "mpu log [failed] [tail: N] [cmd: ПРЕФИКС] [since: КОГДА] [run: ID] [file: ПУТЬ]",
...
Без ключей печатаются последние 20 записей. tail: меняет число;
...
потом хвост tail:. То есть failed tail: 20 — это двадцать последних
УПАВШИХ.
