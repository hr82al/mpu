...
export const logCommand = defineCommand({
  ...
  usage:
    "mpu log [failed] [limit: N] [cmd: ПРЕФИКС] [since: КОГДА] [run: ID] [file: ПУТЬ]",
  help: `Звать, когда надо вспомнить, что и с каким результатом mpu уже
выполнял: журнал хранит строку, вывод и код каждого вызова.

Печатает записи журнала вызовов дословно, от старых к новым:
шапка, секции вывода, строка \`--- end … ---\` и пустая строка между
записями. Своего форматирования и машинных режимов нет — журнал и так
текст.

Без ключей печатаются последние 20 записей. limit: меняет число;
...
потом хвост limit:. То есть failed limit: 20 — это двадцать последних
УПАВШИХ.
...
запись и обрабатывается первым: ни прочие отборы, ни limit: к нему не
...
run: не найден; 2 — ошибки ввода.`,
  examples: [
    "mpu log",
    "mpu log failed limit: 5",
    "mpu log cmd: sql since: 2h",
