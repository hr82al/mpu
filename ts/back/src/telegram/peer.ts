...
export type Peer =
  ...
  /**
   * Вид объявлен пользователем («@», ссылка `t.me`, «+» перед цифрами):
   * Telegram резолвит такую строку сам, и второй попытки у неё нет.
   */
  | { readonly kind: "name"; readonly name: string }
  /**
   * Голая строка, похожая на имя пользователя: сперва штатный резолв,
   * при его неудаче — поиск по названию
...
/** Телефон: ведущий «+» перед цифрами — иначе это числовой id. */
const PHONE = /^\+\d+$/;
...
export function parsePeer(target: string): Peer {
  ...
  // Ведущий «+» перед цифрами объявляет телефон так же, как «@» — имя:
  // подмена ответа Telegram про номер на «совпадений нет» увела бы от
  // настоящей причины, а чат с названием-номером — редкость
  // (`platform/telegram-mtproto.md`, «Резолв адресата»).
  if (PHONE.test(name)) return { kind: "name", name };
  // Голая строка, похожая на имя: имя такое может и не существовать, а
  // чат с таким названием — вполне («news», «team»). Отсюда две попытки.
  if (USERNAME.test(name)) return { kind: "guess", name };
