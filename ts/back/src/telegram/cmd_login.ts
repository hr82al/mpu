...
export const telegramLoginCommand = defineCommand({
  ...
  help: `Звать один раз, когда команды telegram отказывают из-за
отсутствия сессии: интерактивный вход от имени пользователя. Сессия
сохраняется в env-файл, и остальные команды семейства работают без
повторного входа. Та же реализация — шаг входа в mpu init.
...
заводить ключи приложения, пустой ввод, сбой самого входа (неверный
код, недоступная служба, битый прокси, сбой криптографии клиента) — всё
это «пропущено» с кодом 0 и причиной в скобках.
