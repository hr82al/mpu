...
Deno.test("режим дополнения печатает варианты, а не справку", async (t) => {
  ...
  await t.step("группа, переехавшая целиком — её листья", async () => {
    ...
    // Прежде здесь стоял пример смешанного маршрута — группа,
    // уходившая подпроцессом, с переехавшими листьями; таких не
    // осталось (порция 95), а сам маршрут снят (порция 97).
    const { stdout } = await run([], {
      env: (name) =>
        name === "_MPU_COMPLETE"
          ? "complete_bash"
