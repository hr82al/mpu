 ...
 * `entry-unparsed` и `entry-not-object` — вход не определён, и причина
 * названа: манифест пакета либо не разобрался, либо не объект. Обе не
 * сливаются с «входа нет» и друг с другом: молчаливая подмена соседним
 * ответом запрещена, а валидный JSON-массив манифестом не является.
