...
export const codeRefsCommand = defineCommand({
  ...
  usage: "mpu code refs address: АДРЕС [limit: N]",
  help: `Спрашивают перед тем, как переименовать или удалить символ: кто за
него держится и что сломается от правки. Ответ либо полон, либо сам
называет, где неполон, — и этим отличается от текстового поиска,
который не обещает ни того, ни другого.

address: — [РЕПОЗИТОРИЙ:]ПУТЬ[:СТРОКА]. Со строкой — потребители
...
и потребителей символа спросить не у чего. Отказ
печатается разделом — с отметкой дерева и причиной, — а не строкой
ошибки.`,
  examples: [
    "mpu code refs address: sl-back:src/orders/mod.ts:42",
    "mpu code refs address: sl-back:src/orders/mod.ts",
  ],
  policy: "ro",
  argsSchema,
