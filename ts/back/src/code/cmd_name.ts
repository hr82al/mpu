...
export const codeNameCommand = defineCommand({
  ...
  usage: "mpu code name name: ИМЯ [in: ОКНО] [limit: N]",
  help: `Спрашивают перед тем, как внести новое имя: не занято ли оно
другим смыслом там, куда вносят. Отвечает каждый репозиторий рабочей
области своим разделом; где разбора по типам нет, раздел отказывает с
названной причиной, а не выдаёт текстовый ответ за полный.

name: — точное имя объявления, без подстановочных знаков.
...
(пустое имя, нет такого репозитория или каталога) — она решается
раньше отказа раздела; 1 — не ответил ни один раздел. Отказ печатается в своём разделе с причиной и ответы
остальных не отменяет; с окном из одного репозитория он совпадает с
отказом команды.`,
  examples: [
    "mpu code name name: daysBetween in: sl-back",
    "mpu code name name: daysBetween in: sl-back:src/orders",
  ],
  policy: "ro",
  argsSchema,
