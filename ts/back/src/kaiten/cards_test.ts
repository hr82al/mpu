...
Deno.test("вызов 3: комментарии карточки", async (t) => {
  // Порядок ответа сервером не задан (`kaiten-api-cards.md`, «Инварианты»):
  // каталог отдаёт элементы как пришли и хронологии не обещает — сортировка
  // по `created` принадлежит вызывающему.
  await t.step("в порядке ответа, с GFM-разметкой", async () => {
    const second = {
      id: 3002,
      text: "> цитата\n\nи вывод",
