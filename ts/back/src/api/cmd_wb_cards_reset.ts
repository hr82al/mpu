...
переклейке — маркетплейс не обновляет отметку времени карточки при
...
либо сама цель имеет форму sid (UUID) — кэш тогда не открывается
вовсе. Иначе селектор резолвится по кэшу, как в mpu search.

Несколько кабинетов у цели — отказ с перечнем и требованием
