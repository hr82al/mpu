/**
 * Копии фикстур в `testdata/` обязаны совпадать с каналом спецификаций
 * байт-в-байт. Без этой сверки расхождение молчит: тесты продолжают
...
/**
 * Закрытый список публикации — не эталон для сверки, а данные, которые
 * читает сам код, поэтому лежит рядом с ним, а не в testdata. Копия у
 * него одна, и она под той же сверкой с каналом.
 */
const POLICIES = "tool-policies.json";
...
Deno.test("копии фикстур совпадают с каналом спецификаций", async (t) => {
  ...
  await t.step(POLICIES, async () => {
    assertEquals(
      await Deno.readTextFile(new URL(POLICIES, import.meta.url)),
      await Deno.readTextFile(new URL(POLICIES, channelDir)),
    );
  });
});
