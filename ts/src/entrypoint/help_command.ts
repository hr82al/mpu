...
/** Записи обоих маршрутов в порядке реестра плюс сама `mpu help`. */
export function helpEntries(
  ...
  return [
    ...
    { name: "mpu help", summary: HELP_SUMMARY },
  ];
}

const HELP_SUMMARY = "список всех команд mpu с опциональной справкой";
...
export async function runHelpCommand(
  ...
  if (entry.command === undefined) {
    // Запись без обеих поверхностей — только `mpu help` сама: её
    // подробная справка и есть эта однострока.
