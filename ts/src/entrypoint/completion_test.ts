...
Deno.test("режим дополнения печатает варианты, а не справку", async (t) => {
  ...
  await t.step("группа, переехавшая целиком — её листья", async () => {
    // Дополнение внутри группы перечисляет её листья из реестра.
    // Прежде здесь стоял пример смешанного маршрута — группа на
    // `legacy` с переехавшими листьями; таких в реестре не осталось
    // (порция 95), и текст приведён к тому, что проверка делает
    // сейчас.
    const { stdout } = await run([], {
      env: (name) =>
        name === "_MPU_COMPLETE"
    ...
    // Порядок — порядок реестра, а не алфавит: дополнение печатает
    // листья в том виде, в каком они объявлены (`platform/registry.md`).
    assertEquals(stdout.split("\n").filter(Boolean), [
      "send",
      "log",
      "login",
