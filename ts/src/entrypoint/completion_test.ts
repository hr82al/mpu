...
Deno.test("режим дополнения печатает варианты, а не справку", async (t) => {
  ...
  await t.step("частично переехавшая группа — её native-листья", async () => {
    ...
    // группа с тем же профилем: её листья переехали все, включая вход
    // (`registry/mod.ts`, `commands`), а сама группа своего
    // промежуточного узла в `groups` не имеет — в слепке она по-прежнему
    // одна запись верхнего уровня (`registry/legacy_tree.ts`), и
    // неизвестная подкоманда уходит прежней реализации.
    const { stdout } = await run([], {
      env: (name) =>
        name === "_MPU_COMPLETE"
          ? "complete_bash"
