...
Deno.test("mpu help <имя>: справка целевой команды", async (t) => {
  ...
  await t.step("справку legacy-команды печатает подпроцесс", async () => {
    ...
    assertEquals(await cli.run("help", "mpu sql-ro"), 0);
    // Тот же вызов, что у `mpu sql-ro --help`: реестр текст не сочиняет.
    assertEquals(calls, [["sql-ro", "--help"]]);
...
Deno.test("mpu help: неизвестное имя — exit 2 и список известных", async (t) => {
  const cases: readonly (readonly [string, string])[] = [
    ...
    ["пустая строка", ""],
    ["голый kebab вместо полного имени", "sql-ro"],
  ];
