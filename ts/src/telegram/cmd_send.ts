...
const argsSchema = z.object({
  ...
  chat: z.string().optional().describe(
    "адресат: me, id, @username, ссылка t.me или телефон",
  ),
...
--chat X — адресат: me («Избранное»), числовой id, @username, ссылка
t.me или телефон. Не задан — берётся TELEGRAM_DEFAULT_CHAT.

--md — текст и подпись размечены Markdown: [текст](url) становится
ссылкой, **жирный** — жирным. Без флага разметка остаётся видимыми
символами.
...
Отправка необратима и видна получателю: сообщение уходит от твоего имени.
Безопасный адресат для проверок — me.

stdout — одна строка JSON: {"id": …, "chat_id": …, "date": …}.

Ключи env-файла: TELEGRAM_API_ID, TELEGRAM_API_HASH, TELEGRAM_SESSION
(обязательны; строку сессии пишет mpu init), TELEGRAM_DEFAULT_CHAT,
TELEGRAM_PROXY.

Exit: 0 — успех; 1 — конфигурация или отказ Telegram; 2 — ошибка ввода
(адресат, пустой текст, вложение).
