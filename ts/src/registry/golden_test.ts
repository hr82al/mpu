...
Deno.test("справку команды маршрута legacy печатает подпроцесс", async (t) => {
  ...
  // Образец — сама группа `telegram`: её листья переехали все, включая
  // вход, но имя группы осталось в слепке, и неизвестная подкоманда
  // по-прежнему уходит подпроцессу. Прежние образцы, `sheet` и `api`,
  // ведут себя так же — список подпроцессных имён тает с каждой
  // поставкой.
  const cases: readonly (readonly [string, readonly string[], string[]])[] = [
    ...
    // Вход переехал на маршрут `native` (`telegram-login.md`), поэтому
    // образцом подпроцессной подкоманды стало любое неизвестное имя:
    // группа осталась в слепке, и такое имя уходит прежней реализации.
    ["mpu telegram wat --help", ["telegram", "wat", "--help"], [
      "telegram",
      "wat",
      "--help",
    ]],
