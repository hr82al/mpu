...
const CASES: readonly CommandCase[] = [
  ...
  // Копирования. Образец — для инвариантов рендера и сериализации.
  //
  // Условие, которое обязано выполняться, а не просто наблюдение: вызов
  // отбивается **до** pg_dump и до docker, потому что у обхода нет ни
  // кэша с клиентом, ни ключей подключений. С тех пор как у шва redis
  // появилось умолчание (`redisRunner`), дойди вызов до шагов 5-6 — он
  // позвал бы настоящий `docker exec`, вплоть до `FLUSHALL` на локальном
  // redis. Второй слой защиты — узкий `--allow-run` задачи `test`
  // (`deno.jsonc`): docker в нём не значится. Оба слоя нужны; `deno test
  // -A` руками снимает второй.
  {
    path: "copy-client",
    argv: ["5175"],
    sampleResult: {
  ...
  })),
  {
    ...
    path: "telegram login",
    argv: [],
    sampleResult: { status: "skipped", reason: "нет TTY" },
  },
  {
