 ...
 *
 * Одно задание считается на месте: у вопроса об одном репозитории —
 * обычного `--in РЕПО` — максимум и так равен сумме, а воркер не
 * бесплатен. Платит в нём не запуск потока, а повторная загрузка
 * компилятора: `mentions`, который программ не строит, четырьмя
 * воркерами стоит 0.06 с против 0.00 с на месте, тогда как `name` по
 * одному репозиторию — 1.31 с на месте против 1.78 с по трём воркерами
 * (замеры 2026-09-09).
 */
