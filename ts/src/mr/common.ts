/**
 * Общее для read-подкоманд `mpu mr` (`docs/specs/mr-read.md`): доступ к
 * GitLab, резолв `--mr` и перевод отказов атома в ошибки команды.
 *
 * Копия на команду значила бы пять мест правки при изменении текста
 * подсказки или класса ошибки — а подсказка здесь несёт больше, чем
