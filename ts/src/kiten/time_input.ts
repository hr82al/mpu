 ...
 * «CLI-контракт»): длительность, календарная дата и «сегодня» по МСК.
 *
 * Модуль чист — ни сети, ни диска; часы приходят параметром, поэтому
 * «сегодня» проверяется без подмены глобального времени. Отдельно от
 * команд он лежит потому, что те же три вещи нужны и подкомандам
 * таймера: `--time` у `stop` — та же длительность с другим именем
 * аргумента, а московский день считает предупреждение о сдвинутой дате.
 */
...
/** Сдвиг московской зоны: она фиксирована и перевода часов не знает. */
...
/**
 * Сегодняшний день по МСК — умолчание даты записи. Считается от момента в
 * миллисекундах, а не от локальной зоны машины: та у пользователя может
 * быть любой, а записи компании живут в московском дне.
 */
export function mskToday(nowMs: number = Date.now()): string {
