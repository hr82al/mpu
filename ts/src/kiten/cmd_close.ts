...
Порядок шагов фиксирован: таймер → поля → ответ → перенос. Отката между
шагами нет: у Kaiten нет транзакций, поэтому сбой позднего шага
оставляет ранние применёнными, а повторный запуск безопасно доводит
остальное — записанные поля пропускаются как заполненные, перенос
повторяется релогом.

--hypothesis/--done/--result/--mr — значения полей карточки. Пишутся
только переданные и только в пустое поле; заполненное пропускается,
--force-fields перезаписывает. Порядок обработки и вывода фиксирован:
hypothesis, done, result, mr.

Ответ — ровно один источник: --reply TEXT либо --reply-file PATH ('-' —
stdin); пустой текст отвергается. Самостоятельный '@all' в тексте
раскрывается в логин владельца карточки; владельца нет —
предупреждение в stderr и '@all' как есть.

--column REF — колонка переноса (id либо название), по умолчанию ключ
${COLUMN_ENV_KEY} env-файла, иначе «${DEFAULT_COLUMN}». Колонка резолвится
на доске карточки СРАЗУ, до единой мутации. --no-move — не переносить.

--stop-timer — остановить идущий таймер и записать время (роль: ключ
${ROLE_ENV_KEY} → 12058). Без флага таймер не трогается никогда, а о
запущенном таймере команда громко предупреждает.

--dry-run — печать плана; выполняются только чтения.

Ключи env-файла: KITEN_API_KEY (обязателен), KITEN_BASE_URL,
${COLUMN_ENV_KEY}, ${ROLE_ENV_KEY}.

Exit: 0 — успех; 1 — ошибка API Kaiten (в тексте назван упавший шаг);
2 — ошибка ввода (селектор, источники ответа, колонка, ненастроенный
KITEN_API_KEY).
