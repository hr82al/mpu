...
пустая или относительная XDG_CONFIG_HOME равна незаданной), абсолютный
путь. Кандидат подходит, если в нём есть deno.jsonc и main.ts; ни один
не подошёл — отказ называет все три места.

Успешная установка перезаписывает build-source путём собранного дерева
и печатает строку «запомненное дерево: <путь>»; следующий mpu build
находит дерево из любого каталога. --check и неудачная установка файл не
трогают; не удалась только запись — строка «запомненное дерево: не
записано (<причина>)», установка остаётся, exit 0.
