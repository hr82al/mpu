...
Deno.test("пометка «без записи вывода» — часть объявления команды", async (t) => {
  ...
  await t.step("в реестре пометка стоит у восьми команд", () => {
    // Все восемь печатают то, чему в журнале не место: `search` — живые
    ...
    // живой токен sl-back (`docs/specs/api.md`), а две `api list-*` —
    // чужие секреты клиента: WB-токены и Ozon-ключи. Порядок — порядок
    // реестра.
