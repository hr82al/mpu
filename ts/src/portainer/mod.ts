...
/**
 * GET на путь Portainer API с ключом в заголовке `X-API-Key`, тело
 * разобрано как JSON. Код вне 2xx, сбой транспорта и нечитаемое тело —
 * `PortainerError` с причиной одной строкой; ключ в неё не попадает.
 *
 * Пределы времени — параметр со значением по умолчанию, а не константа
 * внутри: тесты задают их маленькими числами и потому укладываются в
 * доли секунды вместо реальных пределов (`ts/CLAUDE.md`: сон стеной в
 * тестах запрещён).
 */
