...
/**
 * Разница между JSON-значением `null` и SQL NULL на этом слое не видна:
 * драйвер отдаёт оба одним и тем же `null`, и колонка, где лежало
 * JSON-`null`, получит SQL NULL. Отличить их можно было бы только по
 * сырому тексту ответа, которого у нас нет; случай назван здесь, чтобы
 * он не выглядел недосмотром.
 *
...
export function paramOf(value: unknown, oid: number): unknown {
  ...
  // Сериализуется **любое** значение, а не только объект и массив.
  // Разобранная строка — тоже разобранное значение: колонка со
  // значением `"привет"` приходит строкой `привет`, и отдать её как
  // есть значит отправить серверу текст, который не JSON. Хуже того,
  // строка, внутри которой лежит текст JSON, при этом молча сменила бы
  // тип: сервер разобрал бы её объектом вместо строки.
