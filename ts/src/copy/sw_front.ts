...
/**
 * Строковый литерал SQL: единственная кавычка удваивается. Здесь ещё
 * литералы, а не параметры, и это осознанно: шаг 6 подставляет только
 * строки, которые сам же и собрал (почта вида `client-<id>@…`,
 * фиксированный хэш) либо прочитал из соседней таблицы (sid и название
 * кабинета). Перенос строк клиента, где значение бывает любым типом и
 * любой формы, на параметры переведён — см. `rows.ts`.
 */
