...
export interface EndpointSpec {
  ...
  /**
   * Ответ несёт то, чему не место на диске: чужие секреты (WB-токены,
   * Ozon-ключи клиента) либо персональные данные и одноразовые ссылки
   * (почта пользователя, `activationLink`). Такая команда не пишет
   * вывод в журнал вызовов: на stdout содержимое остаётся — за ним
   * команду и зовут, — а в журнал не ложится (`ts/CLAUDE.md`: секреты,
   * токены, пароли и персональные данные в вывод и логи не попадают ни
   * в каком виде).
   *
   * Признак один на оба случая, а не два рядом: решаются они
   * одинаково, и второй признак развёл бы то, что не расходится.
   * Пометка стоит в строке таблицы, а не списком имён где-то рядом:
   * список неизбежно разошёлся бы с таблицей.
   */
  readonly sensitiveOutput?: true;
  /**
   * Эндпоинт не требует авторизации: вызов идёт без заголовка и **не
   * трогает кэш токена** — ни читает, ни обновляет. Признак снят с
   * объекта Python-реализации (`api-write.md`), и у `auth-login` он не
   * удобство, а необходимость: брать токен, чтобы сходить за токеном,
   * — порочный круг.
   */
  readonly noAuth?: true;
  /**
   * Среди полей команды есть секрет: пароль, токен, ключ. Тогда
   * аргументы вызова не пишутся в журнал — иначе секрет лёг бы на диск
   * строкой `$ mpu api … --password …` (`ts/CLAUDE.md`: секреты в
   * вывод и логи не попадают ни в каком виде).
   *
   * Признак отдельный от `sensitiveOutput`: у него другая сторона
   * вызова. Команда может нести секрет во входе и не нести в ответе
   * (`auth-change-password`), и наоборот (`auth-refresh`).
   */
  readonly secretInput?: true;
}
