...
export interface EndpointSpec {
  ...
  /**
   * Ответ несёт чужие секреты — токены и ключи клиента к чужим
   * системам. Такая команда не пишет вывод в журнал вызовов: на stdout
   * секрет остаётся (за ним команду и зовут), а на диск в журнал не
   * ложится (`ts/CLAUDE.md`: секреты, токены и пароли в логи не
   * попадают ни в каком виде). Пометка в строке таблицы, а не список
   * имён где-то рядом: список разошёлся бы с таблицей.
   */
