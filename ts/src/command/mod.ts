 ...
 * Исключение — `progress` ниже: служебный канал хода исполнения, не
 * проекция результата команды; сама команда в него не печатает, печать
 * — дело точки входа, как и для остального вывода. Согласованность с
 * инвариантом 1 — `platform/command-contract.md`, инвариант 1: вывод, не
 * являющийся проекцией результата, доставляется портом io и печатается
 * точкой входа в stderr.
 */
