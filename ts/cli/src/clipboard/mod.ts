...
/**
 * Программы копирования — **абсолютными путями**, тот же список, что
 * перечислен в правах задач `cli`/`compile:cli` (`deno.jsonc`). Здесь
 * он единственный: тест прав сверяет строку задачи с ним, а не
 * повторяет пути.
 *
 * Пути, а не имена: имя в `--allow-run` Deno разрешает по `PATH` при
 * старте, и с пустым `PATH` программа падает до первой своей строки
 * (замер 2026-09-22, `cli-client.md`, «Права клиента и `PATH`»).
 * Цена названа честно: на машине, где программа лежит не в `/usr/bin`,
 * копирование не удастся — текст уйдёт в stderr, тому же получателю,
 * что и у клиента в конвейере.
 */
